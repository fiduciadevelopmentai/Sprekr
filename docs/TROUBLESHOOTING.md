...
- **Accessibility is enabled in System Settings but onboarding still says it is not allowed:** macOS is trusting a different Sprekr binary than the one that is running. Common cause: an ad-hoc `build/debug/Sprekr.app` (`com.klimtalks.app.development`) sits beside the installed `/Applications/Sprekr.app` (`com.klimtalks.app`), so Settings shows two Sprekr rows. Quit Sprekr normally, open only `/Applications/Sprekr.app`, enable that row, and leave or remove the development row. Then run `./scripts/install.sh --source` (stale build apps are cleaned up by default). Never reset TCC.
